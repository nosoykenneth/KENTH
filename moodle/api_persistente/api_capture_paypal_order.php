<?php
/**
 * api_capture_paypal_order.php - v2.0
 * Captura una orden de PayPal desde el servidor, valida metadata local,
 * evita reprocesos y matricula al usuario en Moodle.
 */

define('CLI_SCRIPT', false);

require_once(__DIR__ . '/../../config.php');
require_once($CFG->libdir . '/enrollib.php');
require_once($CFG->dirroot . '/user/lib.php');
require_once('tesis_lib.php');

header('Access-Control-Allow-Origin: *');
header('Access-Control-Allow-Methods: POST, OPTIONS');
header('Access-Control-Allow-Headers: Content-Type, Authorization');
header('Content-Type: application/json; charset=utf-8');

if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
    http_response_code(200);
    exit;
}

global $DB, $CFG;

/**
 * Lee variables desde .env local
 */
function get_backend_env($key) {
    $envPath = __DIR__ . '/.env';

    if (!file_exists($envPath)) {
        return null;
    }

    $lines = file($envPath, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);

    foreach ($lines as $line) {
        if (strpos(trim($line), '#') === 0) {
            continue;
        }

        if (strpos($line, '=') !== false) {
            list($name, $value) = explode('=', $line, 2);
            if (trim($name) === $key) {
                return trim($value);
            }
        }
    }

    return null;
}

/**
 * Logging simple
 */
function paypal_log($message) {
    file_put_contents(
        __DIR__ . '/log_paypal_capture.txt',
        '[' . date('Y-m-d H:i:s') . '] ' . $message . PHP_EOL,
        FILE_APPEND
    );
}

/**
 * Template de email unificado estilo KENTH
 */
function build_kenth_enrollment_email($firstname, $courseFullname, $email, $loginUrl, $plaintextPassword = null) {
    $passwordSectionHtml = '';

    if (!empty($plaintextPassword)) {
        $passwordSectionHtml = "
        <div style='margin-top: 20px; padding-top: 20px; border-top: 1px solid rgba(255,255,255,0.1);'>
            <div style='color: rgba(255,255,255,0.4); font-size: 11px; text-transform: uppercase; letter-spacing: 1.5px; margin-bottom: 5px;'>Contraseña Temporal</div>
            <div style='font-size: 20px; font-weight: 700; color: #C3073F; letter-spacing: 1px;'>{$plaintextPassword}</div>
            <div style='color: #ffb800; font-size: 11px; margin-top: 5px;'>* Se te pedirá cambiarla en tu primer acceso.</div>
        </div>";
    }

    return "
    <div style='background-color: #0d0d0f; padding: 40px 20px; font-family: \"Segoe UI\", Helvetica, Arial, sans-serif;'>
        <div style='max-width: 600px; margin: 0 auto; background-color: #1A1A1D; border-radius: 24px; overflow: hidden; border: 1px solid rgba(255,255,255,0.1); box-shadow: 0 20px 40px rgba(0,0,0,0.5);'>
            <div style='background: linear-gradient(135deg, #6F2232 0%, #C3073F 100%); padding: 40px; text-align: center;'>
                <img src='https://i.imgur.com/BXiN2dO.png' alt='KENTH Academy' style='max-width: 200px; height: auto;'>
            </div>

            <div style='padding: 40px; color: #ffffff;'>
                <h2 style='margin-top: 0; font-size: 24px; font-weight: 700;'>¡Bienvenido a la comunidad!</h2>
                <p style='font-size: 16px; line-height: 1.6; color: rgba(255,255,255,0.8);'>
                    Hola <span style='color: #C3073F; font-weight: 700;'>{$firstname}</span>, nos alegra confirmarte que tu pago ha sido procesado con éxito y ya tienes acceso total a tu formación.
                </p>

                <div style='background-color: rgba(255,255,255,0.05); border-radius: 20px; padding: 30px; margin: 30px 0; border: 1px solid rgba(255,255,255,0.05);'>
                    <div style='margin-bottom: 20px;'>
                        <div style='color: rgba(255,255,255,0.4); font-size: 11px; text-transform: uppercase; letter-spacing: 1.5px; margin-bottom: 5px;'>Curso Matriculado</div>
                        <div style='font-size: 18px; font-weight: 600; color: #ffffff;'>{$courseFullname}</div>
                    </div>

                    <div style='margin-bottom: 20px;'>
                        <div style='color: rgba(255,255,255,0.4); font-size: 11px; text-transform: uppercase; letter-spacing: 1.5px; margin-bottom: 5px;'>Usuario de Acceso</div>
                        <div style='font-size: 16px; font-weight: 500; color: #ffffff;'>{$email}</div>
                    </div>

                    {$passwordSectionHtml}
                </div>

                <div style='text-align: center; margin: 40px 0;'>
                    <a href='{$loginUrl}' style='background-color: #C3073F; color: #ffffff; padding: 18px 35px; border-radius: 15px; text-decoration: none; font-weight: 700; font-size: 16px; display: inline-block; box-shadow: 0 10px 20px rgba(195, 7, 63, 0.3);'>
                        EMPEZAR A APRENDER
                    </a>
                </div>

                <p style='font-size: 13px; line-height: 1.5; color: rgba(255,255,255,0.5); text-align: center; margin-top: 40px;'>
                    Si tienes alguna duda, responde a este correo y nuestro equipo te ayudará de inmediato.
                </p>
            </div>

            <div style='padding: 30px; background-color: #121214; text-align: center; border-top: 1px solid rgba(255,255,255,0.05);'>
                <p style='margin: 0; font-size: 12px; color: rgba(255,255,255,0.3);'>
                    &copy; 2026 KENTH Academy. Desarrollado con pasión por la tecnología.
                </p>
            </div>
        </div>
    </div>";
}

try {
    $raw = file_get_contents('php://input');
    paypal_log('HIT api_capture_paypal_order.php');
    paypal_log('INPUT RAW: ' . $raw);

    $data = json_decode($raw, true);

    if (!$data || !is_array($data)) {
        throw new Exception('Payload JSON inválido o vacío.');
    }

    $orderID = trim($data['orderID'] ?? '');
    $clientTransactionId = trim($data['clientTransactionId'] ?? '');
    $courseIdRaw = $data['course_id'] ?? '';
    $emailFromRequest = strtolower(trim($data['email'] ?? ''));
    $firstnameFromRequest = trim($data['firstname'] ?? 'Estudiante');
    $lastnameFromRequest = trim($data['lastname'] ?? 'KENTH');

    if (!$orderID || !$clientTransactionId || !$courseIdRaw || !$emailFromRequest) {
        throw new Exception('Faltan parámetros requeridos (orderID, clientTransactionId, course_id, email).');
    }

    paypal_log("Iniciando captura para OrderID={$orderID} | clientTransactionId={$clientTransactionId} | email={$emailFromRequest}");

    // 1) Buscar metadata local igual que PayPhone
    $paymentsDir = __DIR__ . '/payments';
    $localFile = $paymentsDir . '/' . $clientTransactionId . '.json';

    paypal_log('LOCAL FILE LOOKUP: ' . $localFile);

    if (!file_exists($localFile)) {
        $foundFiles = glob($paymentsDir . '/*.json');
        $fileList = $foundFiles ? array_map('basename', $foundFiles) : [];

        throw new Exception(
            'No se encontró metadata para ' . $clientTransactionId .
            '. Archivos disponibles: ' . implode(', ', $fileList)
        );
    }

    $localData = json_decode(file_get_contents($localFile), true);
    $metadata = $localData['metadata'] ?? null;

    if (($localData['status'] ?? '') === 'approved') {
        paypal_log("TRANSACCIÓN YA PROCESADA: {$clientTransactionId}");

        echo json_encode([
            'success' => true,
            'message' => 'Pago ya procesado anteriormente.',
            'orderID' => $orderID,
            'clientTransactionId' => $clientTransactionId,
            'status' => 'COMPLETED',
            'alreadyProcessed' => true
        ], JSON_UNESCAPED_UNICODE);
        exit;
    }

    if (!$metadata) {
        throw new Exception('No se encontró el objeto metadata en el registro local.');
    }

    if (empty($metadata['email']) || empty($metadata['course_id'])) {
        $missing = [];
        if (empty($metadata['email'])) $missing[] = 'email';
        if (empty($metadata['course_id'])) $missing[] = 'course_id';

        throw new Exception('Datos incompletos en registro local. Faltan: ' . implode(', ', $missing));
    }

    // 2) Tomar datos reales desde metadata local
    $course_id_secure = $metadata['course_id'];
    $email = strtolower(trim($metadata['email']));
    $firstname = trim($metadata['firstname'] ?? $firstnameFromRequest ?: 'Estudiante');
    $lastname = trim($metadata['lastname'] ?? $lastnameFromRequest ?: 'KENTH');

    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        throw new Exception('Correo inválido en metadata local: ' . $email);
    }

    // Defensa extra: el email recibido debe coincidir con el registrado localmente
    if ($emailFromRequest !== $email) {
        throw new Exception('El email enviado no coincide con la metadata registrada para esta transacción.');
    }

    // 3) Obtener credenciales PayPal
    $clientId = get_backend_env('PAYPAL_CLIENT_ID');
    $clientSecret = get_backend_env('PAYPAL_CLIENT_SECRET');
    $mode = get_backend_env('PAYPAL_MODE') ?: 'sandbox';
    $baseUrl = ($mode === 'live')
        ? 'https://api-m.paypal.com'
        : 'https://api-m.sandbox.paypal.com';

    if (!$clientId || !$clientSecret) {
        throw new Exception('Credenciales de PayPal no configuradas en el backend.');
    }

    // 4) Obtener Access Token
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "{$baseUrl}/v1/oauth2/token");
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_POST, true);
    curl_setopt($ch, CURLOPT_USERPWD, "{$clientId}:{$clientSecret}");
    curl_setopt($ch, CURLOPT_POSTFIELDS, 'grant_type=client_credentials');
    curl_setopt($ch, CURLOPT_HTTPHEADER, [
        'Accept: application/json',
        'Accept-Language: es_ES',
        'Content-Type: application/x-www-form-urlencoded'
    ]);

    $tokenResponse = curl_exec($ch);

    if ($tokenResponse === false) {
        $curlError = curl_error($ch);
        $curlErrno = curl_errno($ch);
        curl_close($ch);
        throw new Exception("Error cURL obteniendo token PayPal: [{$curlErrno}] {$curlError}");
    }

    $tokenHttpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
    curl_close($ch);

    $tokenData = json_decode($tokenResponse, true);
    $accessToken = $tokenData['access_token'] ?? null;

    paypal_log("TOKEN RESPONSE HTTP {$tokenHttpCode}: {$tokenResponse}");

    if ($tokenHttpCode < 200 || $tokenHttpCode >= 300 || !$accessToken) {
        throw new Exception('No se pudo obtener el token de acceso de PayPal.');
    }

    // 5) Capturar la orden
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "{$baseUrl}/v2/checkout/orders/{$orderID}/capture");
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_POST, true);
    curl_setopt($ch, CURLOPT_HTTPHEADER, [
        'Content-Type: application/json',
        'Authorization: Bearer ' . $accessToken,
        'PayPal-Request-Id: ' . $clientTransactionId
    ]);

    $captureResponse = curl_exec($ch);

    if ($captureResponse === false) {
        $curlError = curl_error($ch);
        $curlErrno = curl_errno($ch);
        curl_close($ch);
        throw new Exception("Error cURL al capturar orden PayPal: [{$curlErrno}] {$curlError}");
    }

    $captureHttpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
    curl_close($ch);

    $captureData = json_decode($captureResponse, true);

    paypal_log("CAPTURE RESPONSE HTTP {$captureHttpCode}: " . $captureResponse);

    if ($captureHttpCode < 200 || $captureHttpCode >= 300 || !$captureData || !is_array($captureData)) {
        throw new Exception('Error al capturar la orden en PayPal. HTTP ' . $captureHttpCode . '. Respuesta: ' . ($captureResponse ?: 'Vacía'));
    }

    $status = $captureData['status'] ?? '';
    if ($status !== 'COMPLETED') {
        throw new Exception("El pago no pudo ser completado. Estado: {$status}");
    }

    // 6) Resolver curso
    $course_id = kenth_verify_id($course_id_secure);
    if (!$course_id) {
        $course_id = (int)$course_id_secure;
    }

    if (empty($course_id)) {
        throw new Exception('No se pudo resolver el ID del curso.');
    }

    $course = $DB->get_record('course', ['id' => $course_id], '*', MUST_EXIST);

    // 7) Crear o recuperar usuario
    $user = $DB->get_record('user', [
        'email' => $email,
        'deleted' => 0
    ]);

    $is_new_user = false;
    $plaintext_password = null;

    if (!$user) {
        $is_new_user = true;
        $plaintext_password = bin2hex(random_bytes(4)) . 'Kt!';

        $newuser = new stdClass();
        $newuser->username = $email;
        $newuser->email = $email;
        $newuser->firstname = $firstname;
        $newuser->lastname = $lastname;
        $newuser->auth = 'manual';
        $newuser->mnethostid = $CFG->mnet_localhost_id;
        $newuser->confirmed = 1;
        $newuser->lang = 'es';

        $newuser->id = user_create_user($newuser);

        // Forma oficial de Moodle para setear password manual de forma segura
        update_internal_user_password($newuser, $plaintext_password);

        $user = $DB->get_record('user', ['id' => $newuser->id], '*', MUST_EXIST);
        set_user_preference('kenth_requires_onboarding', '1', $user->id);

        paypal_log("USUARIO NUEVO CREADO: {$email} con password set via API");
    } else {
        paypal_log("USUARIO EXISTENTE: {$email}");
    }

    // 8) Enviar email solo si es usuario nuevo
    if ($is_new_user) {
        $subject = '¡Tu matrícula en KENTH Academy está lista!';
        $login_url = 'http://localhost:5173/login';

        $html_message = build_kenth_enrollment_email(
            $firstname,
            $course->fullname,
            $email,
            $login_url,
            $plaintext_password
        );

        $mail_result = email_to_user(
            $user,
            core_user::get_support_user(),
            $subject,
            strip_tags($html_message),
            $html_message
        );

        if ($mail_result) {
            paypal_log("EMAIL ENVIADO EXITOSAMENTE A: {$email}");
        } else {
            paypal_log("FALLO EL ENVÍO DE EMAIL A: {$email}. Verifica la configuración SMTP de Moodle.");
        }
    } else {
        paypal_log("EMAIL OMITIDO: usuario ya existente {$email}");
    }

    // 9) Matricular si no está matriculado aún
    $context = context_course::instance($course->id);
    $alreadyEnrolled = is_enrolled($context, $user->id);

    if (!$alreadyEnrolled) {
        $enrol = enrol_get_plugin('manual');
        $instances = enrol_get_instances($course->id, true);

        $manualInstance = null;
        foreach ($instances as $instance) {
            if ($instance->enrol === 'manual') {
                $manualInstance = $instance;
                break;
            }
        }

        if (!$manualInstance) {
            throw new Exception('No se encontró instancia de matrícula manual para el curso.');
        }

        $roleid = $DB->get_field('role', 'id', ['shortname' => 'student']);
        if (!$roleid) {
            throw new Exception('No se encontró el rol student.');
        }

        $enrol->enrol_user($manualInstance, $user->id, $roleid);
        paypal_log("USUARIO MATRICULADO: {$email} en curso {$course->id}");
    } else {
        paypal_log("USUARIO YA MATRICULADO: {$email} en curso {$course->id}");
    }

    // 10) Marcar registro local como aprobado
    $localData['status'] = 'approved';
    $localData['confirmed_at'] = date('Y-m-d H:i:s');
    $localData['paypal'] = [
        'orderID' => $orderID,
        'clientTransactionId' => $clientTransactionId,
        'status' => $status,
        'captureHttpCode' => $captureHttpCode,
        'captureData' => $captureData
    ];

    file_put_contents(
        $localFile,
        json_encode($localData, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE)
    );

    echo json_encode([
        'success' => true,
        'message' => $is_new_user ? 'Usuario creado y matriculado' : 'Pago validado y matrícula confirmada',
        'orderID' => $orderID,
        'clientTransactionId' => $clientTransactionId,
        'status' => $status,
        'alreadyProcessed' => false
    ], JSON_UNESCAPED_UNICODE);

} catch (Throwable $e) {
    paypal_log('ERROR: ' . $e->getMessage());

    echo json_encode([
        'success' => false,
        'error' => $e->getMessage()
    ], JSON_UNESCAPED_UNICODE);
}