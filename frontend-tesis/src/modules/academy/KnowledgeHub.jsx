 ...
 */
export default function KnowledgeHub({ courseId, axes = [] }) {
  const [summary, setSummary] = useState({ total: 0, global: EMPTY_COUNTS, by_axis: {} });
  const [globalDocs, setGlobalDocs] = useState([]);
  const [structured, setStructured] = useState({ course: [], axes: {}, global_docs: [] });
  const [loading, setLoading] = useState(true);
  const [busy, setBusy] = useState(false);
  const [openKey, setOpenKey] = useState(null);

  // subida. uploadTarget: { scope:'global'|'course'|'axis', axis_id? }
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSummary(sum || { total: 0, global: EMPTY_COUNTS, by_axis: {} });
      setGlobalDocs(gdocs || []);
      setStructured(struct || { course: [], axes: {}, global_docs: [] });
  ...
  const submitUpload = async (e) => {
    ...
    try {
      let res;
      if (uploadTarget?.scope === 'axis') {
        // Recurso formal de EJE (soporta todos los formatos, incl. audio/plantilla).
        res = await uploadAxisResource(courseId, uploadTarget.axis_id, {
          file,
          title: form.title,
  ...
  const removeAxisResource = async (axisId, r) => {
    if (!window.confirm(`¿Eliminar el recurso del eje "${r.title}"?`)) return;
    setBusy(true);
    try {
      await deleteAxisResource(courseId, axisId, r.doc_id);
      showNotification('success', 'Recurso del eje eliminado.');
      await load();
    } catch (e) { showNotification('error', e.message); }
    finally { setBusy(false); }
  };
  ...
  const renderUploadForm = () => {
    ...
    const isAxis = sc === 'axis';
    const headerTxt = sc === 'global'
      ? 'Subir a GLOBAL (todos los cursos)'
      : isAxis ? `Recurso del ${uploadTarget?.axis_id}` : 'Recurso del curso completo';
    ...
    <form onSubmit={submitUpload} className="mt-2 border border-kenth-border rounded-xl p-3 bg-kenth-surface/5 flex flex-col gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        <div>
          <label className={labelCls}>{isAxis ? 'Archivo (pdf/img/audio/plantilla)' : (docType === 'imagen' ? 'Imagen (png/jpg/webp)' : 'Archivo (pdf/txt/md)')}</label>
      ...
      <div>
        ...
        <textarea rows={docType === 'imagen' ? 4 : 2} className={inputCls} value={form.description}
          ...
          placeholder={isAxis ? 'Qué es y para qué sirve (obligatorio para audio/plantilla si se indexa)' : (docType === 'imagen' ? 'Ej: captura del compresor X…' : 'Contexto extra para el tutor (opcional)')} />
      </div>
  ...
  const curso = summary.by_axis?.['(sin eje)'];
  const cursoItems = curso?.items || [];
  const totalCounts = Object.values(summary.by_axis || {}).reduce(
    ...
    <div className="flex flex-col gap-4">
      ...
      <div>
        ...
        <div className="flex flex-col gap-1.5">
          {axes.map((ax) => {
            const counts = summary.by_axis?.[ax.axis_id] || EMPTY_COUNTS;
            const open = openKey === ax.axis_id;
            const items = summary.by_axis?.[ax.axis_id]?.items || [];
            const sx = structured.axes?.[ax.axis_id] || { axis_resources: [], lessons: {} };
            const axisResources = sx.axis_resources || [];
            ...
            return (
              <div key={ax.axis_id} className="border border-kenth-border rounded-xl bg-kenth-card">
                <div className="flex items-center gap-2 px-3 py-2">
                  <button onClick={() => setOpenKey(open ? null : ax.axis_id)} className="flex-1 text-left min-w-0">
                    <span className="text-sm font-bold text-kenth-text">{open ? '▾' : '▸'} {ax.axis_id}</span>
                    <span className="text-kenth-subtext text-xs"> · {ax.axis_title || ax.title}</span>
                    <div className="mt-0.5">
                      <CountLine c={counts} />
                      <span className="text-[10px] text-kenth-subtext ml-2">· eje {axisResources.length} · lecciones {lessonCount}</span>
                    </div>
                  </button>
                </div>
                {open && (
                  <div className="px-3 pb-3 flex flex-col gap-3">
                    {/* A) Recursos propios del EJE (scope='axis') */}
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <p className={labelCls}>🧩 Recursos del eje</p>
                        <button onClick={() => startUpload({ scope: 'axis', axis_id: ax.axis_id })}
                          className="text-[10px] font-black uppercase text-kenth-brightred hover:underline">+ Recurso del eje</button>
                      </div>
                      {axisResources.length === 0
                        ? <p className="text-[11px] text-kenth-subtext">Sin recursos propios del eje. Aplican a todas sus lecciones.</p>
                        : <div className="flex flex-col gap-1.5">{axisResources.map((r) => (
                            <ResourceRow key={r.doc_id} r={r} onDelete={(rr) => removeAxisResource(ax.axis_id, rr)} />
                          ))}</div>}
                      {uploadTarget?.scope === 'axis' && uploadTarget?.axis_id === ax.axis_id && renderUploadForm()}
                    </div>
          ...
          <div className="border border-kenth-border rounded-xl bg-kenth-card">
            <div className="flex items-center gap-2 px-3 py-2">
              <button onClick={() => setOpenKey(openKey === '__curso__' ? null : '__curso__')} className="flex-1 text-left">
                <span className="text-sm font-bold text-kenth-text">{openKey === '__curso__' ? '▾' : '▸'} 📁 Curso completo (sin eje)</span>
              ...
              <div className="px-3 pb-3 flex flex-col gap-3">
                ...
                <div>
                  ...
                  {(structured.course || []).length === 0
                    ? <p className="text-[11px] text-kenth-subtext">Sin recursos a nivel de curso (sin eje).</p>
