...
import {
  listPilotLessons,
  getPilotLesson,
  buildPilotActivityContext,
} from '../../shared/services/pilotService';
import { createChatSession } from '../../shared/services/aiService';

/**
 * Panel de prueba de la vertical slice del tutor contextual.
 *
 * Permite:
 *  1. Elegir una de las 3 lecciones piloto.
 *  2. Fijar un timestamp con un slider.
 *  3. Ver el bloque resuelto en el cliente (mismo criterio que el backend).
 *  4. Lanzar preguntas al tutor con activity_context piloto.
 *
 * No reemplaza la UI principal: vive como ruta /dashboard/pilot.
 */
...
export default function PilotTutorDebugView() {
  const [pilotIndex, setPilotIndex] = useState([]);
  ...
  const userId = localStorage.getItem('moodle_userid') || 'pilot-debug-user';
  ...
  useEffect(() => {
    listPilotLessons()
      ...
      .catch((e) => console.error('[PILOT]', e))
      .finally(() => setLoading(false));
  }, []);

  useEffect(() => {
    if (!selectedId) return;
    getPilotLesson(selectedId)
      .then((data) => {
        setLesson(data);
        setTimestamp(0);
      })
  ...
  const currentBlock = useMemo(() => findBlockAt(lesson, timestamp), [lesson, timestamp]);

  const activityContext = useMemo(
    () => buildPilotActivityContext(lesson, timestamp),
    [lesson, timestamp],
  );
  ...
  const ensureSession = async () => {
    ...
    try {
      const created = await createChatSession(
        `Piloto ${selectedId} ${new Date().toLocaleTimeString()}`,
      );
      setSessionId(created.id);
      return created.id;
    } catch (e) {
      console.error('[PILOT] No se pudo crear sesion, usando id anonimo', e);
      const fallback = `pilot-${Date.now()}`;
  ...
  if (loading) {
    return (
      <div className="p-6 text-sm text-kenth-subtext">Cargando lecciones piloto...</div>
    );
  }

  if (!pilotIndex.length) {
    return (
      <div className="p-6 text-sm text-red-400">
        No hay lecciones piloto cargadas en course_runtime/pilot/.
      </div>
    );
  }
